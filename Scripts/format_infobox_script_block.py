...
DATA_PATH = os.path.join(SCRIPT_PATH, "..", "pz-scripts-data", "out", "scriptBlocks.json")
DATA_PATH = os.path.abspath(DATA_PATH)
SCRIPTS_DOCS_LINK = r"https://pz-wiki-modding.github.io/PZ-API-Docs/scripts/{1}.html"
...
def make_socials(type, link, text):
    socials = """    {{Infobox/socials
    | type=${type}
    | link=${link}
    | ${text}
    }}"""
    socials = socials.replace("${type}", type)
    socials = socials.replace("${link}", link)
    socials = socials.replace("${text}", text)
    return socials
...
# make socials
socials = make_socials("pzapidocs", SCRIPTS_DOCS_LINK.replace("{1}", block_name), "ScriptsDocs")


section = section.replace("${rows}", "\n".join(rows))
infobox = infobox.replace("${socials}", socials).strip()
...
# to wikitext via pandoc
description_wt = subprocess.run(
    ["pandoc", "-f", "markdown", "-t", "mediawiki"],
    input=description,
    capture_output=True,
    text=True
)
description_wt = pzwiki.mediawiki_to_pzwiki(description_wt.stdout.strip())

out += "\n" + description_wt.strip()
